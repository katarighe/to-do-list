...
const getmyTask = () => {
  ...
  const myTaskElement = myTask.map((item) => `
        <li class="to-do-list to-do-item" id=${item.index}>
          ${item.completed === true ? `
            <i class="fa-solid fa-check checked-icon" title="Mark task as incomplete"></i>`
    : '<i class="fa-solid fa-square unchecked-icon" title="Mark task as complete"></i>'}
          <input type="text" class=${item.completed === true ? 'decoration edit-todo' : ' edit-todo'}  value="${item.description}" title="Edit this task">
